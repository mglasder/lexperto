...
def paragraph_extraction_sv_node(state: SectionText) -> GraphState:
    openai = init_chat_model(
        "openai:gpt-4.1-mini",
        temperature=0.0,
    )
    ...
    agent = create_react_agent(
        ...
        prompt=INSTRUCT_PARAGRAPHS.format(
            section_name="Sachverhalt",
            numbering_logic=SV_PARA_LOGIC,
            paragraph_struct=Paragraph.model_fields,
            example_input=EXAMPLE_INPUT_SV,
            example_output=EXAMPLE_OUTPUT_SV,
        ),
        response_format=(
    ...
    section = Section(
        name="sachverhalt", content=response["structured_response"].paragraphs
    )

    return {"sections": [section]}


def paragraph_extraction_erw_node(state: SectionText) -> GraphState:
    openai = init_chat_model(
        "openai:gpt-4.1-mini",
        temperature=0.0,
    )
    ...
    section = Section(
        name="erwaegungen", content=response["structured_response"].paragraphs
    )
    return {"sections": [section]}
